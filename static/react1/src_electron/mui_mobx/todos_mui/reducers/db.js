  ...
  emit: function(url, data, callback) {
    ...
    if (url == '/folder') {
      ...
      db.serialize(function() {
        ...
        db.all(
          "SELECT * FROM parts_pack where name like '%" +
            data.search +
            "%' limit " +
            data.limit +
            ' offset ' +
      ...
      db.serialize(function() {
        ...
        db.all(
          "SELECT * FROM parts_item where name like '%" +
            data.search +
            "%' limit " +
            data.limit +
            ' offset ' +
      ...
      var cmd =
        'SELECT a.id,a.ct,a.quehuo,b.name,b.guige,b.bh,b.id as item_id FROM parts_packitem as a,parts_item as b where a.item_id=b.id and  a.pack_id=' +
        data.pack_id +
        ' limit ' +
        data.limit +
        ' offset ' +
      ...
      console.log(data);
      var cmd =
        'insert into  parts_usepack(contact_id,pack_id) values(' +
        data.contact_id +
        ',' +
        data.pack_id +
        ')';
      ...
      console.log(data);
      var cmd =
        'insert into  parts_packitem(pack_id,item_id,ct,quehuo) values(' +
        data.pack_id +
        ',' +
        data.item_id +
        ',1,0)';
    ...
    ///delete/UsePack
      ...
      if (!baoxiang)
        cmd =
          "select strftime('%Y-%m',tiaoshi_date) as month,count(id) as total from parts_contact  where tiaoshi_date between '" +
          start_date_s +
          "' and '" +
          end_date_s +
          "' group by month";
        ...
        cmd =
          "select strftime('%Y-%m',tiaoshi_date) as month,count(id) as total from parts_contact  where baoxiang like '" +
          baoxiang +
