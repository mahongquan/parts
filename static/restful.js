...
Ext.define('ui.LoginForm', {
    ...
    initComponent: function () {
        ...
        var tokenTxt = {
            ...
            , name: 'csrfmiddlewaretoken', // fieldLabel:this.passwordFL,
            // labelWidth:this.labelWidth,
            //width:this.fieldWidth,
            // labelAlign:this.labelAlign,
            // inputType:'password',
            value: myglobal.token, //allowBlank: false,
            //blankText:'不可空白',
            // minLength: 4,
            //minLengthText:'最少{0}個字元',
            //msgTarget:'under'
    ...
    onSubmitClickhandler: function (btn, evt) {
        ...
        this.submit({
            ...
            failure: function (form, action) {
                    this.getEl().unmask();
                    var alertConfig = {
                        title: '錯誤', //icon: Ext.window.MessageBoxWindow.ERROR,
                        buttons: Ext.Msg.OK
                        , msg: ''
                    };
...
Ext.define('Item', {
    extend: 'Ext.data.Model', //clientIdProperty: 'clientId',
    fields: [{
        name: 'id'
        , type: 'int'
        , useNull: true
...
var loadglobal = function () {
    ...
    myglobal.loadContact = function () {
        ...
        var grid = Ext.create('Ext.grid.Panel', {
            ...
            frame: true, //title: '合同',
            store: store, //iconCls: 'icon-contact',
            columns: [{
                    text: 'ID', //width: 50,
                    sortable: true
                    , dataIndex: 'id'
                        ...
                        , handler: function () {
                            ...
                            if (selection) {
                                var r = Ext.create('PackItem', {
                                    pack: packid
                                    , itemid: selection.data.id, //name:selection.data.name,
                                    ct: 1
                                , });
                                dest_store.add(r);
                                dest_store.sync();
...
Ext.onReady(function () {
    ...
    var showLogin = function () {
        var helloWindow = new Ext.Window({
            ...
            , modal: true
        , });
        var loginform = new ui.LoginForm({
            url: "/rest/login/"
            , token: myglobal.token
            , win: helloWindow
            , userDiv: userDiv
        });
